...
main challenge is to \emph{label} the backbone so that (i) merged flows can actually travel (there is a temporal path between every terminal pair on the backbone) and (ii) each edge carries only a few labels, so almost no
...
polynomial-time $O(n^{2/3+\epsilon})$-approximation for DSF, for any fixed $\epsilon>0$, which our algorithm uses as a black box.
