...
Minimum Aged Labelling (MAL), introduced by Carnevale et al., asks for the cheapest set of labels that
keeps a temporal graph connected. In its basic form the travel time of every edge is one time unit,
and there is an upper bound $a$ on the labels an edge may use.
...
\noindent Each label is one departure, so MAL minimizes the number of departures that keeps everyone
reachable from everyone in time.

\subsection{Minimum Aged Labelling with capacity constraints}\label{sec:malc}

MAL charges one unit per label, implicitly assuming that a single departure suffices to carry whatever
must cross an edge at that time. In practice demands are heterogeneous: the amount that must cross an
edge at a given time may require several unit-capacity vehicles. \emph{Minimum Aged Labelling with
Capacity constraints} (MAL-C) captures this by charging, for each (edge, time) pair, the number of
vehicles its flow needs.
...
\begin{equation}\label{eq:treerec}
  \din(x)=w(x,x')+\din(x')\ \ \text{for an in-tree edge }(x,x'),\qquad
