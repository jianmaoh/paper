...
is \emph{temporally connected} if, for every ordered pair of vertices $(u,v)$,
  there is a temporal path from $u$ to $v$.
\end{definition}

\subsection{The Minimum Labelling problem}\label{sec:mal}

The Minimum Labelling (ML) problem, introduced by Klobas et al.~\cite{KMMS24}, asks for the fewest
labels that make a graph temporally connected; additionally bounding the largest label gives the
Minimum Aged Labelling (MAL) problem of Carnevale et al.~\cite{CDO25}. Our problem builds on ML---we
impose no bound on the labels---generalised to edge travel times and augmented with capacity.

\begin{problem}[Minimum Labelling (ML)]\label{prob:ml}
\leavevmode
\begin{description}[topsep=2pt,itemsep=1pt,leftmargin=6.5em,style=nextline]
  \item[\textnormal{Input:}] a digraph $G=(V,E)$.
  \item[\textnormal{Output:}] a label function $\lambda:E\to 2^{\Zzero}$.
  \item[\textnormal{Constraint:}] $(G,\lambda)$ is temporally connected.
...
The variant we study augments ML with edge travel times $w$, a demand set with real-valued amounts,
and the capacity objective; like ML it imposes no maximum-label bound (a label may be any time in
$\Zzero$), and all demands are available from time $0$. Flow is splittable, and nodes have unlimited
buffering: a demand may wait at a node for any amount of time.
  ...
  \item[\textnormal{Constraints:}]
    \begin{enumerate}[topsep=1pt,itemsep=1pt,label=(\roman*)]
      ...
      \item \emph{Flow conservation:} each $f_i$ routes $\dem(d_i)$ units from $\src(d_i)$ to
            $\dst(d_i)$, respecting travel times and using unlimited node buffering.
    \end{enumerate}
...
still counts as one dispatch (the \emph{round-up}). Node buffering is free: only edge dispatches are
charged.
...
All waste in \eqref{eq:objective} comes from the round-up. Two observations drive the algorithm. First, \emph{integer amounts never waste}: if a demand
...
every remainder were shipped on its own it would need a whole vehicle per edge. We therefore
\emph{consolidate} the fractional remainders of many demands onto one shared network so that they
travel together and share vehicles.
...
main challenge is to \emph{label} the backbone so that (i) merged flows can actually travel on temporal paths between every terminal pair on the backbone and (ii) each edge carries only a few labels, so almost no
rounding occurs. We build the backbone from a Directed Steiner Forest.
...
solution \emph{reuses} edges across terminal pairs rather than giving each pair its own path. This
small shared edge set lets different demands' fractional flows consolidate onto common edges, and its
small size keeps the backbone's own dispatch cost low (\S\ref{sec:ratio}).

\noindent Berman, Bhattacharyya, Makarychev, Raskhodnikova and Yaroslavtsev~\cite{BBMRY13} give a
polynomial-time $O(n^{2/3+\epsilon})$-approximation for DSF, for any fixed $\epsilon>0$.

%====================================================================
\section{The Algorithm}\label{sec:algorithm}
%====================================================================

There are four ingredients in our algorithm---backbone construction, backbone labelling, backbone
routing, and shortest-path routing---and Algorithm~\ref{alg:main} combines them.
...
distance from $x$ to $h_i$, and $\dout(x)$ the distance from $h_i$ to $x$. These are
...
We illustrate $\textsc{Assign-Labels}$ and the resulting canonical routes on a backbone whose graph
$\GSF$ has two SCCs, $C_1$ and $C_2$;
for concreteness each component is a small directed triangle:
\[
...
\[
  T_2=\max\bigl\{\,T_1+1,\ \ T_1+D^{(1)}_{\mathrm{in}}+D^{(1)}_{\mathrm{out}}+w(c,q)\,\bigr\}
     =\max\{\,1,\ 7+2\,\}=9,
\]
so $C_2$ opens no earlier than any flow can arrive through $c\to q$.
...
temporal path. The flow leaves $C_1$ at time $7$ and reaches $q$ at $7+w(c,q)=9=T_2$, matching $C_2$'s
base time.
...
fractional part $d_f\in[0,1)$. The integer part is always sent by $\textsc{Route-SP}$. The fractional part is governed by $\theta$: if
...
\subsection{Temporal paths between all terminal pairs}\label{sec:tempconn}
...
$T_i+\Din+\dout(v)$. In particular there is a temporal path from $u$ to $v$, and the segment occupies the
time window $[\,T_i,\;T_i+\Din+\Dout\,]$.
\end{lemma}

\begin{proof}
By Definition~\ref{def:tpath} the route is a temporal path once we check, edge by edge, that the next
departure never precedes the current arrival, i.e.\ that $t_{i+1}\ge t_i+w(e_i)$. We show that on each
tree edge the arrival coincides with the next label, so this condition holds with equality and no
waiting occurs.

\emph{Ascent.} Along the in-tree path $u=x_0\to\cdots\to x_p=h_i$ with $e_k=(x_k,x_{k+1})\in\Tin$,
\eqref{eq:treerec} gives $\din(x_k)=w(e_k)+\din(x_{k+1})$. Departing $x_k$ at its label
$t_{\mathrm{in}}(e_k)=T_i+\Din-\din(x_k)$, the flow arrives at $x_{k+1}$ at
\[
  t_{\mathrm{in}}(e_k)+w(e_k)=\bigl(T_i+\Din-\din(x_k)\bigr)+w(e_k)=T_i+\Din-\din(x_{k+1})=t_{\mathrm{in}}(e_{k+1}),
\]
exactly the label of the next in-tree edge; the monotonicity condition thus holds with equality on
every ascent edge (no waiting), and the flow reaches $h_i$ at $T_i+\Din$ (since $\din(h_i)=0$).

\emph{Descent.} The first out-tree edge departs $h_i$ at
$t_{\mathrm{out}}(g_0)=T_i+\Din+\dout(h_i)=T_i+\Din$, exactly the arrival at $h_i$ just computed, so the
hand-off at the hub is again an equality. Along the out-tree path $h_i=y_0\to\cdots\to y_q=v$ with
$g_k=(y_k,y_{k+1})\in\Tout$ and $\dout(y_{k+1})=\dout(y_k)+w(g_k)$ by~\eqref{eq:treerec}, departing
$y_k$ at its label $t_{\mathrm{out}}(g_k)=T_i+\Din+\dout(y_k)$ the flow arrives at $y_{k+1}$ at
\[
  t_{\mathrm{out}}(g_k)+w(g_k)=T_i+\Din+\dout(y_{k+1})=t_{\mathrm{out}}(g_{k+1}),
\]
again the next label. Every edge of the route therefore meets $t_{i+1}=t_i+w(e_i)$ with equality, so
the route is a temporal path; it reaches $v$ at $T_i+\Din+\dout(v)\le T_i+\Din+\Dout$.
\end{proof}

\begin{lemma}[Cross edges preserve temporality]\label{lem:cross}
Let $e=(a,b)$ be a cross edge with $a\in C_i$ and $b\in C_j$ ($i<j$). Under the base
times~\eqref{eq:basetime}, the flow arrives at $a$ no later than the label $t_{\mathrm{cross}}(e)=T_i+\Din+\Dout$,
departs $e$ at that label, and arrives at $b$ by time $T_j$. Hence the cross hop and the hand-off into
$C_j$ satisfy the monotonicity condition $t_{i+1}\ge t_i+w(e_i)$---here possibly with strict inequality,
since the flow may wait.
\end{lemma}

\begin{proof}
As in Lemma~\ref{lem:intra} we check the condition $t_{i+1}\ge t_i+w(e_i)$ at each transition; the only
difference is that here it may hold strictly (the flow waits), because $e$ carries the single label
$t_{\mathrm{cross}}(e)$ rather than a matching pair of tree labels.

\emph{Arrival at $a$.} By the canonical route $a$ is the exit vertex of $C_i$, reached by descending the
out-tree, so by the descent in Lemma~\ref{lem:intra} the flow arrives at $a$ at
\[
  T_i+\Din+\dout(a)\;\le\;T_i+\Din+\Dout=t_{\mathrm{cross}}(e),
\]
using $\dout(a)\le\Dout$. Thus the arrival at $a$ is at most the departure label $t_{\mathrm{cross}}(e)$,
so the flow departs $e$ at its label (waiting at $a$ if the inequality is strict).

\emph{Cross hop.} Departing $a$ at $t_{\mathrm{cross}}(e)$, the flow arrives at $b$ at
$t_{\mathrm{cross}}(e)+w(e)$. By~\eqref{eq:basetime}, $T_j\ge T_i+\Din+\Dout+w(e)=t_{\mathrm{cross}}(e)+w(e)$,
so the flow arrives at $b$ by time $T_j$.

\emph{Hand-off into $C_j$.} The first in-tree departure at $b$ is $T_j+D^{(j)}_{\mathrm{in}}-\din(b)\ge T_j$,
at least the arrival $t_{\mathrm{cross}}(e)+w(e)\le T_j$; so the flow waits at $b$ and continues up
$C_j$'s in-tree, again meeting $t_{i+1}\ge t_i+w(e_i)$.
\end{proof}
...
\end{tikzpicture}
\caption{Cross edge $e=(a,b)$, $a\in C_i$, $b\in C_j$ (Lemma~\ref{lem:cross}). Descending $C_i$'s
out-tree, the flow reaches the exit $a$ by $t_{\mathrm{cross}}=T_i+\Din+\Dout$; it departs $e$ at
$t_{\mathrm{cross}}$ and reaches the entry $b$ of $C_j$ by $T_j\ge t_{\mathrm{cross}}+w(e)$, then waits
for $C_j$'s in-tree schedule ($\ge T_j$).}
\label{fig:cross}
\end{figure}

\begin{theorem}[A temporal path for every terminal pair]\label{thm:temporal}
For every terminal pair $(u,v)$ joined through the backbone---within one component or across
several---the canonical route $R(u,v)$ is a temporal path, so all backbone flows can be routed
simultaneously.
\end{theorem}
...
the following cross edge with the hand-off into $C_{i_{l+1}}$ meets the monotonicity condition
$t_{i+1}\ge t_i+w(e_i)$ by Lemma~\ref{lem:cross}, delivering the flow to the next entry vertex by
$T_{i_{l+1}}$. Splicing these sub-walks---each satisfying $t_{i+1}\ge t_i+w(e_i)$ at every edge---yields
a temporal path.
\end{proof}
...
(the whole of $d_f$ if $d_f\le2\theta$, otherwise exactly $\theta$). By Theorem~\ref{thm:twolabels}
every backbone edge carries at most two labels; writing $f_{t_1}(e),f_{t_2}(e)$ for the flows on them
and $F_e=f_{t_1}(e)+f_{t_2}(e)$ for the total flow on $e$, the number of vehicles dispatched on $e$ is
$\lceil f_{t_1}(e)\rceil+\lceil f_{t_2}(e)\rceil\le F_e+2$. Summing over all backbone edges,
...
\end{equation}
Each label of $e$ carries the combined flow of the at most $h$ demands routed through it, each of size
at most $2\theta$, so a single label carries at most $2h\theta$; as $e$ has at most two labels,
$F_e\le 4h\theta$. Hence the flow term satisfies
\begin{equation}\label{eq:flowterm}
  \sum_{e\in\ESF}F_e\;\le\;4h\theta\,|\ESF| .
\end{equation}

Combining \eqref{eq:sfsum}--\eqref{eq:flowterm} with $|\ESF|\le O(n^{2/3})\OPTSF$,
...
The backbone labelling gives every terminal pair a temporal path while putting at most two labels on each edge, so consolidated flows share vehicles and almost no rounding occurs. The bound is round-up
...
\bibitem{KMMS24}
N.~Klobas, G.~B.~Mertzios, H.~Molter, and P.~G.~Spirakis.
\newblock The complexity of computing optimum labelings for temporal connectivity.
\newblock \emph{Journal of Computer and System Sciences}, 146:103564, 2024.
\end{thebibliography}
