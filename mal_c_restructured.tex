...
\begin{description}[topsep=2pt,itemsep=1pt,leftmargin=6.5em,style=nextline]
  ...
  \item[\textnormal{Constraint:}] $(G,\lambda)$ is temporally connected between every ordered pair of
        vertices.
...
times~\eqref{eq:basetime}, the flow reaches $a$ no later than $t_{\mathrm{cross}}(e)=T_i+\Din+\Dout$,
departs $e$ at that label, and reaches $b$ by time $T_j$. Hence the cross hop together with the
...
\]
using $\dout(a)\le\Dout$. Since $t_{\mathrm{cross}}(e)$ is the only label of $e$, the flow (waiting if
necessary) departs $e$ at $t_{\mathrm{cross}}(e)$ and arrives at $b$ at $t_{\mathrm{cross}}(e)+w(e)$.
By~\eqref{eq:basetime}, $T_j\ge T_i+\Din+\Dout+w(e)=t_{\mathrm{cross}}(e)+w(e)$, so the flow reaches
